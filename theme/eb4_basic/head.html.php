...
$disable_nav = false;
if(strpos(strtolower($g5['lo_url']), "/mypage/") == 0 || strpos(strtolower($g5['lo_url']), "/shop/") == 0) {
   //todo 특정 페이지만 include 변경
    if(in_array($g5['page_name'], $mypage_target_pages)) {
		//	jinam23 , 230524 - edited. css 버전수정처리
        add_stylesheet('<link rel="stylesheet" href="'.EYOOM_THEME_URL.'/css/mypage.css?ver='.G5_CSS_VER.'1">',0);
        $disable_nav = true;
    }
}
...
<? include_once __DIR__."/gnb_m.html.php"; ?>

    <?php if(!defined('_INDEX_')) { // 메인이 아닐때 ?>
    <?php if(!$disable_nav) { /*----- page title 시작 -----*/ ?>
...
<script>
	...
	function change6(obj) {
		gnb61.style.display = "none";
		gnb62.style.display = "none";
		gnb63.style.display = "none";
		gnb64.style.display = "none";
		obj.style.display = "block";
	}
</script>