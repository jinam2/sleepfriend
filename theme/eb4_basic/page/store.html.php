<?php
/**
 * page file : /theme/THEME_NAME/page/store.html.php
 */
...
$sql = "select * from sleep_place where category = 'store' order by display_order desc, address_bcode asc ";
...
while($row = sql_fetch_array($res)) {
    ...
    if($row['main_image_file']) {
        $main_img_file = G5_DATA_PATH . '/place/'.$row['main_image_file'];
        if(file_exists($main_img_file)) {
            $row['thumb_url'] = str_replace("\"", "'",get_place_thumbnail($main_img_file, 400, 300, '', false, false));
        }
    }
    $place_list[] = $row;
}
		...
		<div class="store">
			<div class="search">
                ...
                <div class="input input2">
                    <input type="text" name="search_name" value="" placeholder="매장이름">
                    ...
                    <?php foreach($place_list as $row) { ?>
                        <li data-bcode="<?=$row['address_bcode']?>" data-sido="<?=$row['sido']?>" data-sigungu="<?=$row['sigungu']?>" data-lat="<?=$row['address_lat']?>" data-lon="<?=$row['address_lon']?>" data-title="<?=$row['store_name']?>" data-tel="<?=$row['tel']?>">
                            <div class="Item-preview">
                                <div class="Item-preview_mask">
                                    <?php if($row['thumb_url']) { ?>
                                        <div class="Item-preview_image" style="background-image:url(<?=$row['thumb_url']?>);"></div>
                                    <?php } else { ?>
                                        <div class="Item-preview_image"></div>
                                    <?php } ?>
                                </div>
                            </div>
                            ...
                            <div class="Item-detail">
                                <div class="Item-info">
                                    <div class="Item-price"><p><span class="Item-name"><?=$row['store_name']?></span></p></div>
                                </div>
                                <span class="Item-summary address"><?=$row['address1']?> <?=$row['address2']?> </span>
                                <span class="Item-title"><?=$row['tel']?> </span>
                            </div>
                        </li>
                    <?php } ?>
                </ul>
