 ...
 * @file    ~/theme/basic/skin/place/hospitallist.html.php
 */
...
<div class="admin-shop-orderlist">
    <form id="fsearch" name="fsearch" class="eyoom-form" method="get">
        ...
        <div class="adm-headline">
            <h3>병원 목록</h3>
            <a href="<?php echo G5_ADMIN_URL; ?>/?dir=place&pid=hospital_form" class="btn-e btn-e-red btn-e-lg"><i class="fas fa-plus"></i> 주소 추가</a>
        </div>
                                    ...
                                    <div class="margin-bottom-5">
                                        <label class="select form-width-150px">
                                            <select name="sel_field" id="sel_field">
                                                <option value="store_name" <?php echo get_selected($sel_field, 'store_name'); ?>>병원명</option>
                                            </select><i></i>
                                        </label>
                                    </div>
                                    <span>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-dark">
                ...
                <h4 class="modal-title">병원 정보 수정</h4>
            </div>
        ...
        window.db    = db,
            db.clients   = [
                <?php for ($i=0; $i<count((array)$list); $i++) { ?>
                {
                    ...
                    관리: "<?php if ($is_admin!='group') { ?><a href='<?php echo G5_ADMIN_URL; ?>/?dir=place&amp;pid=hospital_form&amp;seq=<?php echo $list[$i]['seq']; ?>&amp;w=u<?php if ($qstr) { ?>&amp;<?php echo $qstr; ?><?php } ?>'><u>수정</u></a><?php } ?>",
                    이미지: "<div style='width:50px;margin:0 auto'><?php echo $list[$i]['thumb']; ?></div>",
                    병원명: "<span class='ellipsis'><a <?php if (!(G5_IS_MOBILE || $wmode)) { ?>href='<?php echo G5_ADMIN_URL; ?>/?dir=place&pid=hospital_form&seq=<?php echo $list[$i]['seq']; ?>&w=u&amp;wmode=1' onclick='eb_modal(this.href); return false;'<?php } else { ?>href='javascript:void(0);'<?php } ?> ><i class='fas fa-external-link-alt color-light-grey margin-right-5 hidden-xs'></i><strong><?php echo $list[$i]['store_name']; ?></strong></a></span>",
                    ...
                    위도_lat: "<?php echo $list[$i]['address_lat']?>",
                    경도_lon: "<?php echo $list[$i]['address_lon']?>",
    ...
    $(document).ready(function(){
        $("#store-list").jsGrid({
            ...
            fields         : [
                ...
                { name: "병원명", type: "text", align: "left", width: 180 },
                { name: "시도", type: "text", align: "center", width: 80 },
                { name: "전화번호", type: "text", align: "center", width: 120 },
                { name: "우편번호", type: "text", align: "center", width: 70 },
                { name: "기본주소", type: "text", align: "left",  width: 200 },
                { name: "상세주소", type: "text", align: "left",  width: '' },
                { name: "위도_lat", type: "text", align: "center",  width: 120 },
                { name: "경도_lon", type: "text", align: "center",  width: 120 },
