        ...
        <div class="table-list-eb">
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        <th class="table-form-th">
                            <label for="salesforce_id" class="label">세일즈포스 환자 아이디</label>
                        </th>
                        <td >
                            <label class="input form-width-250px">
                                <input type="text" name="salesforce_id" id="salesforce_id" value="<?php echo $mb['salesforce_id']; ?>" maxlength="20">
                            </label>
                        </td>
                        <th class="table-form-th">
                            <label for="salesforce_id" class="label">ResMed 환자 아이디</label>
                        </th>
                        <td >
                            <div class="inline-group">
                                <span>
                                    <?php if($airview) { ?>
                                    <label class="input form-width-250px">
                                       [연결됨] 환자명 : <?php echo $airview['username']; ?>
                                    </label>
                                    <?php } else { ?>
                                        연결된 정보 없음
                                    <?php } ?>
                                ...
                                <?php if($airview) { ?>
                                <span><input type="button" name="btn_disconnect_airview" value="ResMed 연결 해제" class="btn-e btn-e-lg btn-e-orange"></span>
                                <div class="note"><strong>Note:</strong> 연결정보를 변경할 경우, 연결 해제후 재 연결하세요.</div>
                                <?php } else { ?>
                                <span><input type="button" name="btn_sync_airview" value="ResMed 연결" class="btn-e btn-e-lg btn-e-green"></span>
                                <div class="note"><strong>Note:</strong> 세일즈포스 환자 아이디를 ResMed에 입력후 연결 버튼을 클릭하세요.</div>
                                <?php } ?>
                    ...
                    <?php } ?>
                        <th class="table-form-th border-left-th">
                            <label for="mb_sex" class="label">성별</label>
                        </th>
                        <td>
                            <div class="inline-group">
                                <label for="mb_sex_none" class="radio"><input type="radio" name="mb_sex" value="" id="mb_sex_none" <?php echo $mb_sex_none; ?>><i></i> 선택안함</label>
                                <label for="mb_sex_man" class="radio"><input type="radio" name="mb_sex" value="M" id="mb_sex_man" <?php echo $mb_sex_man; ?>><i></i> 남성</label>
                                <label for="mb_sex_woman" class="radio"><input type="radio" name="mb_sex" value="F" id="mb_sex_woman" <?php echo $mb_sex_man; ?>><i></i> 여성</label>
                            </div>
                        </td>
                    </tr>
                    <tr>
...
jQuery(function($){
    ...
    $("input[name=btn_sync_airview]").click(function(e) {
        ...
        salesforce_id = $("input[name=salesforce_id]").val();

        if(!salesforce_id) {
            alert("세일즈포스 아이디를 입력하세요.");
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code != 200) {
                    ...
                    if (response.patient) {
                        alert('연동되었습니다. 환자명 : ' + response.patient['username']);
                    } else {
                        alert('환자정보가 없습니다.');
                    }
                }
    ...
    $("input[name=btn_disconnect_airview]").click(function(e) {
        
        $(this).prop('disabled', true);
        var params = {
            action: 'disconenct_airview',
        }
