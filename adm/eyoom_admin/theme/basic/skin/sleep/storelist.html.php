        ...
        window.db    = db,
            db.clients   = [
                <?php for ($i=0; $i<count((array)$list); $i++) { ?>
                {
                    ...
                    관리: "<?php if ($is_admin!='group') { ?><a href='<?php echo G5_ADMIN_URL; ?>/?dir=sleep&amp;pid=store_form&amp;seq=<?php echo $list[$i]['seq']; ?>&amp;w=u<?php if ($qstr) { ?>&amp;<?php echo $qstr; ?><?php } ?>'><u>수정</u></a><?php } ?>",
                    매장명: "<span class='ellipsis'><a <?php if (!(G5_IS_MOBILE || $wmode)) { ?>href='<?php echo G5_ADMIN_URL; ?>/?dir=sleep&pid=store_form&seq=<?php echo $list[$i]['seq']; ?>&w=u&amp;wmode=1' onclick='eb_modal(this.href); return false;'<?php } else { ?>href='javascript:void(0);'<?php } ?> ><i class='fas fa-external-link-alt color-light-grey margin-right-5 hidden-xs'></i><strong><?php echo $list[$i]['store_name']; ?></strong></a></span>",
