...
<div class="admin-shop-orderlist">
    <form id="frmcontractlist" name="frmcontractlist" class="eyoom-form" method="get">
        ...
        <div class="adm-headline">
            <h3>처방내역</h3>
            <a href="javascript:;" id="btn_reload_prescription" class="btn-e btn-e-red btn-e-lg"><i class="fas fa-refresh"></i> salesforce 처방 내역 갱신</a>
        </div>
                                    ...
                                    <div class="margin-bottom-5">
                                        <label class="select form-width-150px">
                                            <select name="sel_field" id="sel_field">
                                                ...
                                                <option value="ID" <?php echo get_selected($sel_field, 'ID'); ?>>처방ID</option>
                                                <option value="CONTRACT_ID" <?php echo get_selected($sel_field, 'CONTRACT_ID'); ?>>계약ID</option>
                                            </select><i></i>
                                        </label>
                                    </div>
                                    <span>
        ...
        window.db    = db,
            db.clients   = [
                <?php for ($i=0; $i<count((array)$list); $i++) { ?>
                {
                    ...
                    관리: "<?php if ($is_admin!='group') { ?><a href='<?php echo G5_ADMIN_URL; ?>/?dir=sleep&amp;pid=prescription_form&amp;ID=<?php echo $list[$i]['ID']; ?>&amp;w=u<?php if ($qstr) { ?>&amp;<?php echo $qstr; ?><?php } ?>'><u>수정</u></a><?php } ?>",
                    이름: "<?php echo $list[$i]['mb_name']?>",
                    처방ID: "<?php echo $list[$i]['ID']?>",
                    계약ID: "<?php echo $list[$i]['CONTRACT_ID']; ?>",
                    처방상태: "<?php echo $list[$i]['PRESCRIPTION_STATUS']?>",
                    요양기관: "<?php echo $list[$i]['HOSPITAL']?>",
                    처방발행일: "<?php echo $list[$i]['ISSUE_DATE']?>",
                    처방시작일: "<?php echo $list[$i]['START_DATE']?>",
                    처방만료일: "<?php echo $list[$i]['END_DATE']?>",
                    주치의: "<?php echo $list[$i]['DOCTOR']?>",
                    진료과목: "<?php echo $list[$i]['MEDICAL_DEPARTMENT'];?>",
                    다음진료방문일정: "<?php echo $list[$i]['next_doctor_datetime'];?>",
                    처방값: "<?php echo $list[$i]['PRESCRIPTION_VALUE'];?>",
                    환자ID: "<?php echo $list[$i]['PATIENT_ID'];?>",
                    처방번호: "<?php echo $list[$i]['PRESCRIPTION_NUMBER'];?>",
                },
                <?php } ?>
            ]
    }();
    ...
    $(document).ready(function(){
        $("#order-list").jsGrid({
            ...
            fields         : [
                ...
                { name: "처방ID", type: "text", align: "center", width: 150, },
                { name: "계약ID", type: "text", align: "center", width: 150 },
                { name: "처방상태", type: "text", align: "center",  width: 80 },
                { name: "요양기관", type: "text",  align: "left", width: 150 },
                { name: "처방발행일", type: "number", align: "center",  width: 100 },
                { name: "처방시작일", type: "number", align: "center", width: 100 },
                { name: "처방만료일", type: "text", align: "center",  width: 100 },
                { name: "주치의", type: "text", align: "center", width: 70 },
                { name: "진료과목", type: "text", width: 120 },
                { name: "다음진료방문일정", type: "text",  align: "center", width: 140 },
                { name: "처방값", type: "text", width: 80 },
                { name: "환자ID", type: "text", align: "center", width: 150 },
                { name: "처방번호", type: "text", align: "center", width: 150 },
            ]
        })
...
<script>
    $(function(){
        ...
        btn_reload_prescription

        $("#btn_reload_prescription").click(function(e) {
            if(!confirm('처방 내역 정보를 다시 불러오시겠습니까?')) {
                return;
            }
            e.preventDefault();
