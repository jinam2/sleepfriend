...
<div class="admin-shop-orderlist">
    <form id="frmcontractlist" name="frmcontractlist" class="eyoom-form" method="get">
        ...
        <div class="adm-headline">
            <h3>청구내역</h3>
            <a href="javascript:;" id="btn_reload_invoice" class="btn-e btn-e-red btn-e-lg"><i class="fas fa-refresh"></i> salesforce 청구 내역 갱신</a>
        </div>
        ...
        window.db    = db,
            db.clients   = [
                <?php for ($i=0; $i<count((array)$list); $i++) { ?>
                {
                    ...
                    청구ID: "<?php echo $list[$i]['ID']?>",
                    환자ID: "<?php echo $list[$i]['PATIENT_ID']?>",
                    환자명: "<?php echo $list[$i]['mb_name']?>",
                    청구상태: "<?php echo $list[$i]['STATUS']; ?>",
                    청구시작일: "<?php echo $list[$i]['START_DATE']?>",
                    청구종료일: "<?php echo $list[$i]['END_DATE']?>",
                    공단부담금: "<?php echo $list[$i]['NHIS_PAYABLE']?>",
                    본인부담금: "<?php echo $list[$i]['PATIENT_PAYABLE']?>",
                    계약ID: "<?php echo $list[$i]['CONTRACT_ID']?>",
                    청구번호: "<?php echo $list[$i]['INVOICE_NUMBER']?>",
                    처방전발행일: "<?php echo $list[$i]['PRESCRIPTION_ISSUE_DATE']?>",
                    기타: "",
                },
                <?php } ?>
            ]
    ...
    $(document).ready(function(){
        $("#order-list").jsGrid({
            ...
            fields         : [
                ...
                { name: "환자명", type: "text", align: "center", width: 100, },
                { name: "환자ID", type: "text", align: "center", width: 200, },
                { name: "청구ID", type: "text", align: "center", width: 200, },
                { name: "청구상태", type: "text", align: "center", width: 100 },
                { name: "청구시작일", type: "text", width: 100 },
                { name: "청구종료일", type: "text", align: "center",  width: 100 },
                { name: "공단부담금", type: "number",  align: "right", width: 120 },
                { name: "본인부담금", type: "number",  align: "right", width: 120 },
                { name: "계약ID", type: "text",  align: "center", width: 200 },
                { name: "청구번호", type: "text",  align: "center", width: 140 },
                { name: "처방전발행일", type: "text",  align: "center", width: 120 },
                { name: "기타", type: "text", width: '' },
            ]
        })
...
<script>
    $(function(){
        ...
        $("#btn_reload_invoice").click(function(e) {
            if(!confirm('청구 정보를 다시 불러오시겠습니까?')) {
                return;
            }
            e.preventDefault();
