...
<div class="admin-shop-orderlist">
    ...
    <div class="adm-headline">
        <h3>계약관리</h3>
        <a href="javascript:;" id="btn_reload_contract" class="btn-e btn-e-red btn-e-lg"><i class="fas fa-refresh"></i> salesforce 계약 내역 갱신</a>
    </div>
        ...
        <div class="table-list-eb">
            ...
            <table class="table">
                <tbody>
                                ...
                                <div class="margin-bottom-5">
                                    <label class="select form-width-150px">
                                        <select name="sel_field" id="sel_field">
                                            ...
                                            <option value="ID" <?php echo get_selected($sel_field, 'ID'); ?>>계약ID</option>
                                        </select><i></i>
                                    </label>
                                </div>
                                <span>
                    ...
                    <tr>
                        <th class="table-form-th">
                            <label for="od_status" class="label">계약상태</label>
                        </th>
                        <td>
                            <label class="select form-width-150px">
                    ...
                    <?php } ?>
                        <th class="table-form-th border-left-th">
                            <label for="od_settle_case" class="label">레코드유형</label>
                        </th>
                        <td>
                            <label class="select form-width-150px">
                    ...
                    <tr>
                        <th class="table-form-th">
                            <label class="label">보험구분</label>
                        </th>
                        <td colspan="3">
                            <label class="select form-width-150px">
                                <select name="type_of_insurance" id="type_of_insurance">
                                    <option value="" <?php echo get_selected($type_of_insurance, ''); ?>>전체</option>
                                    <option value="건강보험-일반" <?php echo get_selected($type_of_insurance, '건강보험-일반'); ?>>건강보험-일반</option>
                                    <option value="건강보험–차상위" <?php echo get_selected($type_of_insurance, '건강보험–차상위'); ?>>건강보험–차상위</option>
                                    <option value="의료수급권" <?php echo get_selected($type_of_insurance, '의료수급권'); ?>>의료수급권</option>
                                    <option value="비보험-렌탈" <?php echo get_selected($type_of_insurance, '비보험-렌탈'); ?>>비보험-렌탈</option>
                                    <option value="산재보험" <?php echo get_selected($type_of_insurance, '산재보험'); ?>>산재보험</option>
                                    <option value="구매계약" <?php echo get_selected($type_of_insurance, '구매계약'); ?>>구매계약</option>
                                </select><i></i>
                            </label>
...
!function () {
    ...
    db.clients   = [
        ...
        {
            체크: "<input type='hidden' name='contract_id[<?php echo $i; ?>]' value='<?php echo $list[$i]['ID']; ?>' id='contract_id_<?php echo $i; ?>'><label for='chk_<?php echo $i; ?>' class='checkbox'><input type='checkbox' name='chk[]' value='<?php echo $i; ?>' id='chk_<?php echo $i; ?>'><i></i></label>",
            회원이름: "<?php echo $list[$i]['mb_name']?>",
            ID: "<a href='<?php echo $list[$i]['href']; ?>' target='_blank'><?php echo $list[$i]['ID']; ?></a>",
            보험구분: "<?php echo $list[$i]['TYPE_OF_INSURANCE']?>",
            실제계약일자: "<?php echo $list[$i]['REAL_START_DATE']."~".$list[$i]['REAL_EXPIRE_DATE']?>",
            반납일자: "<?php echo $list[$i]['RETURN_DATE']?>",
            반납사유: "<?php echo $list[$i]['RETRUN_REASON']?>",
            요양기관명: "<?php echo $list[$i]['PRESCRIPTION_HOSPITAL']?>",
            계약제품군: "<?php echo $list[$i]['PRODUCT_FAMILY']?>",
            종류_타입: "<?php echo $list[$i]['OPERATION_TYPE'];?>",
            대여료_지급일자: "<?php echo $list[$i]['RENTAL_FEE_PAYDAY'];?>",
            장치모델: "<?php echo $list[$i]['DEVICE_MODEL_NAME'];?>",
            장치시리얼: "<?php echo $list[$i]['SN'];?>",
            자동이체ID: "<?php echo $list[$i]['PAYMENT_METHOD'];?>",
            마스크주문일자: "<?php echo $list[$i]['MASK_ORDER_DATE'];?>",
            계약유형: "<?php echo $list[$i]['CONTRACT_TYPE'];?>",
        },
        <?php } ?>
    ]
}();
...
$(document).ready(function(){
    $("#order-list").jsGrid({
        ...
        fields         : [
            ...
            { name: "회원이름", type: "text", align: "center", width: 80, },
            { name: "ID", type: "text", align: "center", width: 150 },
            { name: "보험구분", type: "text", width: 100 },
            { name: "실제계약일자", type: "text", align: "center",  width: 180 },
            { name: "반납일자", type: "text",  align: "center", width: 100 },
            { name: "반납사유", type: "text", align:"center", width: 120 },
            { name: "요양기관명", type: "text", align:"center", width: 120 },
            { name: "계약제품군", type: "text", align: "center",  width: 100 },
            { name: "종류_타입/타입", type: "text", width: 100 },
            { name: "대여료_지급일자", type: "text", align:"center", width: 120 },
            { name: "장치모델", type: "text", width: 200 },
            { name: "장치시리얼", type: "text", width: 140 },
            { name: "자동이체ID", type: "text", align:"center", width: 140 },
            { name: "마스크주문일자", type: "text",align:"center", width: 120 },
            { name: "계약유형", type: "text", align:"center", width: 120 },
        ]
    })
...
$(function(){
    ...
    $("#btn_reload_contract").click(function(e) {
        if(!confirm('계약정보를 다시 불러오시겠습니까?')) {
            return;
        }
        e.preventDefault();
