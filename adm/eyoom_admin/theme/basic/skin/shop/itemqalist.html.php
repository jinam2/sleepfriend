...
!function () {
    ...
    db.clients   = [
        <?php for ($i=0; $i<count((array)$list); $i++) {
            $list[$i]['od_href'] =  G5_ADMIN_URL."/?dir=shop&pid=orderform&od_id=". $list[$i]['od_id'];
        ?>
        {
            ...
            상품정보_주문정보: <?php if($list[$i]['od_id']) {?>"<a href='<?php echo $list[$i]['od_href']; ?>' target='_blank' class='font-bold'><span class='color-grey'><?php echo $list[$i]['od_id']; ?></span></a>"<?php } else {?>"<a href='<?php echo $list[$i]['href']; ?>' target='_blank' class='font-bold'><span class='color-grey'><?php echo $list[$i]['it_id']; ?></span><br><?php echo get_text($list[$i]['it_name']); ?></a>"<?php }?>,
            분류: "<?php echo get_text($list[$i]['iq_category']); ?>",
...
$(document).ready(function(){
    $("#admin-shop-itemqalist").jsGrid({
        ...
        fields         : [
            ...
            { name: "상품정보_주문정보", type: "text", width: 200 },
            { name: "분류", type: "text", width: 100 },
