...
for ($i=0; $i<$cnt; $i++)
{
    ...
    //기존 db의 ct_status 가 완료가 아닌 상태에서 완료 상태로 변경된 경우 salesforce 주문 데이타를 호출한다.
    if($ct['ct_status'] != '완료' && $ct_status == '완료'){
        $mb = get_member($od['mb_id']);
        $payment_method =  $od['od_settle_case'] == "신용카드" ? "카드" : "현금";
        if($mb['salesforce_id']) {
            //1 delivery_method 현장=1  /   택배=2
            $salesforce_sync->set_order_data(
                $mb['salesforce_id'],
...
if($cancel_change) {
    ...
    if (isset($_POST['ct_status']) && in_array($_POST['ct_status'], $status_normal)) { // 정상인 주문상태만 기록
        ...
        if($od['od_status'] != '완료' && $_POST['ct_status'] == '완료') { //완료 일자 업데이트
            $sql .= " , od_complete_datetime = now() ";
        }
    }
