...
if($is_upload_file) {
    ...
    for ($i = 3; $i <= $num_rows; $i++) {
        ...
        //분류를 분류코드가 아닌 분류명으로 넣은 경우 처리
        if($ca_id) {
            $sql3 = "select ca_id from {$g5['g5_shop_category_table']} where ca_name = '$ca_id' ";
            $row3 = sql_fetch($sql3);
            if ($row3['ca_id']) {
                $ca_id = $row3['ca_id'];
            }
        ...
        if($ca_id2) {
            $sql3 = "select ca_id from {$g5['g5_shop_category_table']} where ca_name = '$ca_id2' ";
            $row3 = sql_fetch($sql3);
            if($row3['ca_id']) {
                $ca_id2 = $row3['ca_id'];
            }
        ...
        if($ca_id3) {
            $sql3 = "select ca_id from {$g5['g5_shop_category_table']} where ca_name = '$ca_id3' ";
            $row3 = sql_fetch($sql3);
            if($row3['ca_id']) {
                $ca_id3 = $row3['ca_id'];
            }
