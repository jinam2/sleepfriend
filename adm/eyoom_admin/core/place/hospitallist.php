<?php
/**
 * @file    /adm/eyoom_admin/core/place/hostpitallist.php
 */
...
$action_url1 = G5_ADMIN_URL . '/?dir=place&amp;pid=hospitallist_update&amp;smode=1';
...
$sel_field = (isset($_GET['sel_field']) && in_array($_GET['sel_field'], array('store_name', 'place_name', 'title')) ) ? $_GET['sel_field'] : '';
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    if($row['main_image_file']) {
        ...
        if(file_exists($main_img_file)) {
            $row['thumb'] = str_replace("\"", "'",get_place_thumbnail($main_img_file, 50, 50));
        }
    }
