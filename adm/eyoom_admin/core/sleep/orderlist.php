...
$sel_field = (isset($_GET['sel_field']) && in_array($_GET['sel_field'], array('mb_name', 'mb_id','mb_hp', 'ID', 'NAME')) ) ? $_GET['sel_field'] : '';
...
if ($sel_field != "") {
    ...
    if ($sel_field == 'mb_name') {
        ...
        $where[] = " T1.PATIENT IN(select mb_name from g5_member where mb_name like '%{$search}%') ";

    } else if ($sel_field == 'mb_hp') {
        $where[] = " T1.PATIENT IN(select mb_name from g5_member where mb_hp like '%{$search}%') ";
...
if ($fr_date && $to_date) {
    $where[] = " (T1.ORDER_DATE between '$fr_date' and '$to_date') ";
}
...
$sql_common = " from SF_ORDER T1 $sql_search ";
...
{
    $mem = sql_fetch(" select * from {$g5['member_table']} where salesforce_id = '{$row['PATIENT']}' ");
