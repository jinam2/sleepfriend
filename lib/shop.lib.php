...
function print_item_options_price($it_id, $cart_id)
{
    global $g5;
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        if($row['io_price'] >= 0) {
            $str .= display_price($row['io_price']) ."<br/>".PHP_EOL;
        }
    }
    return $str;
}
 ...
 * @return string
 */
function masking_number_format($val, $mask)
{
    $maskared = '';
...
// 상품이미지 썸네일 생성
function get_place_thumbnail($img_path, $width, $height=0, $id='', $is_crop=false, $html=true)
{
    $str = '';
