...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
        ...
        // 휴대폰인증 체크
        if($config['cf_auth_hp'] && $config['cf_req_hp']) {
            $auth_hp_num = get_session('ss_cert_hp');
            if(!$auth_hp_num || no_hyphen_hp_number($mb_hp) != no_hyphen_hp_number($auth_hp_num)) {
        ...
        if($config['cf_auth_hp'] && $member['mb_hp'] != $mb_hp && $config['cf_req_hp']) {
            $auth_hp_num = get_session('ss_cert_hp');
            if(!$auth_hp_num || no_hyphen_hp_number($mb_hp) != no_hyphen_hp_number($auth_hp_num)) {
                alert("휴대폰 번호 변경을 위해서는 휴대폰인증을 해주셔야 합니다.");
            }
        }
    }
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_URL.'/mypage/register_form.php">
        ...
        <script>
        alert("회원 정보가 수정 되었습니다.");
        location.href="'.G5_URL.'/mypage/myinfo.php";
        </script>
        </body>
        </html>';
    }
