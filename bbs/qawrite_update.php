...
$qa_group = get_text($_POST['qa_group']);
...
if($w == '' || $w == 'a' || $w == 'r') {
    ...
    if($w == 'a') {
        ...
        $qa_group = addslashes($write['qa_group']);
        $qa_type = 1;
        $qa_status = 1;
    }
...
if($config['cf_sms_use'] == 'icode' && $qaconfig['qa_use_sms']) {
    if($config['cf_sms_type'] == 'LMS') {
        ...
        // 답변글은 질문 등록자에게 전송
        if($w == 'a' && $write['qa_sms_recv'] && trim($write['qa_hp'])) {
            $sms_content = '상담신청내역'.' '.$qaconfig['qa_title'].'에 답변이 등록되었습니다.';
            $send_number = preg_replace('/[^0-9]/', '', $qaconfig['qa_send_number']);
            $recv_number = preg_replace('/[^0-9]/', '', $write['qa_hp']);
...
if($w == 'a') {
    if($is_admin == "super") {
        $result_url = G5_ADMIN_URL . '/?dir=sleep&pid=qalist' . $qstr;
    } else {
        $result_url = G5_BBS_URL . '/mypage/reservation_view.php?qa_id=' . $qa_id . $qstr;
    }
}  else if($w == 'u' && $write['qa_type']) {
    if($is_admin == "super") {
        $result_url = G5_ADMIN_URL . '/?dir=sleep&pid=qalist' . $qstr;
    } else {
        $result_url = G5_BBS_URL . '/mypage/reservation_view.php?qa_id=' . $write['qa_parent'] . $qstr;
    }
} else {
    if($is_admin == "super") {
        $result_url = G5_ADMIN_URL . '/?dir=sleep&pid=qalist' . $qstr;
    } else {
        $result_url = G5_URL . '/mypage/reservation.php' . preg_replace('/^&amp;/', '?', $qstr);
    }
}

if($w == "" && $member['salesforce_id'] && $qa_2) { //salesforce 상담신청 연동
    ...
    include_once G5_PATH . "/lib/MediUtil.php";
    ...
    $start_datetime = $qa_2;
    $end_datetime = date("Y-m-d H:i:s", strtotime('+30 minutes',strtotime($start_datetime)));

    $dt = new DateTime($start_datetime, new DateTimeZone('Asia/Seoul'));
    $dt2 = new DateTime($end_datetime, new DateTimeZone('Asia/Seoul'));
    $dt->setTimezone(new DateTimeZone('UTC'));
    $dt2->setTimezone(new DateTimeZone('UTC'));
    $start_tz_time =  $dt->format('Y-m-d\TH:i:s').".000Z";
    $end_tz_time =  $dt2->format('Y-m-d\TH:i:s').".000Z";

    $salesforce_sync->set_reservation_data(
        $member['salesforce_id'],
        $qa_category,
        $qa_group,
        $qa_1,
        $qa_subject,
...
if($w == 'a') {
    ...
    // 답변 알림 메세지
    //https://sleepfriend.co.kr/mypage/reservation_view.php?qa_id=23
    $push = new OneSignalPush();
    $push_message = "작성하신 상담문의에 답변이 등록되었습니다.";
    $target_url = G5_URL."/mypage/reservation_view.php?qa_id=".$qa_id;
    $push->sendQaReplyNotification($write['mb_id'], $push_message, $target_url);
}
