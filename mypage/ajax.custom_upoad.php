...
try {
    ...
    @mkdir(G5_DATA_PATH.'/item/proposal', G5_DIR_PERMISSION);
    @chmod(G5_DATA_PATH.'/item/proposal', G5_DIR_PERMISSION);

    $upload_dir = '/item/proposal/'.date('Ymd');
    ...
    if(is_uploaded_file($tmp_file)) {
        ...
        $sql = "SELECT * FROM g5_shop_proposal_file where ct_id={$ct_id}";
        $row = sql_fetch($sql);

        if($row['ct_id']) {
            //todo 기존 파일 삭제  처리
            $realpath = G5_DATA_PATH."/".ltrim($row['bf_file'], "/");
            if(file_exists($realpath) && is_file($realpath)) {
                @unlink($real_path);
            }

            sql_query("delete from g5_shop_proposal_file where ct_id ={$ct_id}");
        }
        ...
        $bf_ext = $ext;
        $sql = "INSERT INTO g5_shop_proposal_file
                    SET ct_id = '{$ct_id}',
                        bf_no = 0,
                        bf_source = '{$bf_source}',
                        bf_file = '{$bf_file}',
